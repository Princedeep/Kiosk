...
//		setLayout(new BorderLayout());
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Use that input to add a new row to the model by populating the above Object
				// array
				// When the array is populated call the model.addRow() method to insert the new
				// row
			}
		});
		 ...
		 * DefaultTableModel model = (DefaultTableModel) table.getModel(); int
		 ...
		 * 
		 * 
		 * model.removeRow(table.getSelectedRow()); }catch(Exception ex){
		 * ex.printStackTrace(); }
		 * 
		 * }
		 * 
